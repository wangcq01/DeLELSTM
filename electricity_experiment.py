...
data_path=r'C:DATA\electricity'
...
y_train_t = torch.Tensor(train_Y['consumption'].to_numpy()).reshape(len(train_idx), depth-1)
y_val_t = torch.Tensor(val_Y['consumption'].to_numpy()).reshape(len(val_idx), depth-1)
y_test_t = torch.Tensor(test_Y['consumption'].to_numpy()).reshape(len(test_idx), depth-1)
...
save_path = os.path.join('results', 'electricity/decompose')
...
for exp_i in range(5):
    ...
    epoch_scheduler =torch.optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.1, patience=10, verbose=False,
    ...
    for i in range(epochs):
        ...
        if min_val_loss > mse_val ** 0.5:
            ...
            print("Saving...")
            torch.save(model.state_dict(), "decompose_electricity.pt")
    ...
    # test data
    model.load_state_dict(torch.load('decompose_electricity.pt'))
    with torch.no_grad():
        preds = []
        true = []
        ...
        for batch_x, batch_y in test_loader:
            batch_x = batch_x.cuda()
            batch_y = batch_y.cuda()
            output, unorm = model(batch_x.float())
